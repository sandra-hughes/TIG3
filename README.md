...
If a page is refreshed or closed during a game, the current run is saved. Next time, the player confirms restore first, then a 3-second countdown runs before play resumes. Breakout and Snake throttle routine saves to about once per second, validate saved snapshots before restore, force-save on lifecycle events, and auto-pause when the pointer leaves the game area.
